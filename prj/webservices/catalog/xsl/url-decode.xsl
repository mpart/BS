...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template name="decode-query">
  	<xsl:param name="query"/>
  	<xsl:variable name="query-without-plus-signs"><xsl:call-template name="replace-string"><xsl:with-param name="text" select="$query"/><xsl:with-param name="from" select="'+'"/><xsl:with-param name="to" select="' '"/></xsl:call-template></xsl:variable>
  	<xsl:call-template name="decode"><xsl:with-param name="encoded" select="$query-without-plus-signs"/></xsl:call-template>
  </xsl:template>
